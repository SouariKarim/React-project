import moment from 'moment/moment'; // handling dates

class DisponibilityMarker {
  constructor({ code, disponibility, date }) {
    this._code = code;
    this._disponibility = disponibility; // boolean
    this._date = date.date ? new Date(date.date) : new Date(date); // the date in the argument is an object
  }

  get code() {
    // return the disponibility code : see the switch in the label method below
    return this._code;
  }
  ...
  // this is a method without the getter , we can use it anyway
  // return the days left to become disponible
  date_to_days_ago(date) {
    ...
    const now = moment();
    const diffDays = Math.round(now.diff(date, 'days', true)); // diff is a moment package method
  ...
  // format the days left for disponibility in a proper string
  get dateFormatted() {
    ...
    return (
      ...
      year +
      ' (il y a ' +
      this.date_to_days_ago(this.date) +
      ') '
    );
  }

  // reuturn a proper labes message based on the code and the formatted date
  get label() {
    switch (this._code) {
      case 'MARKER_A_LECOUTE_ESN_PARTENAIRE':
        return (
          "Renseigné A L'ECOUTE par une ESN partenaire le " + this.dateFormatted
        );
      case 'MARKER_VU_DISPONIBLE_JOBBOARD':
        return (
          "S'est mis DISPONIBLE sur un site d'emploi le " + this.dateFormatted
        );
      case 'MARKER_RENSEIGNE_DISPONIBLE_ESN':
        return (
          'Renseigné DISPONIBLE par une ESN partenaire le ' + this.dateFormatted
        );
      case 'MARKER_OPEN_TO_WORK_LK':
        return 'OPEN TO WORK sur LinkedIn depuis le ' + this.dateFormatted;
      case 'MARKER_NOT_OPEN_TO_WORK_LK':
        return (
          "N'est plus OPEN TO WORK sur LinkedIn depuis le " + this.dateFormatted
        );
      ...
      case 'MARKER_VIEW_AVAILABLE_ON_JM':
        return (
          "S'est signalé DISPONIBLE sur Jean-Michel.io le " + this.dateFormatted
        );
      case 'MARKER_VIEW_NOT_AVAILABLE_ON_JM':
        return (
          "S'est signalé NON DISPONIBLE sur Jean-Michel.io le " +
          this.dateFormatted
        );
      ...
      case 'MARKER_UPDATE_BY_JM_MODERATOR':
        return (
          'Indiqué disponible par un modérateur JM le ' + this.dateFormatted
        );
      case 'MARKER_VIEW_AVAILABLE_ON_JP':
        return (
          "S'est signalé DISPONIBLE sur Jean-Paul.io le " + this.dateFormatted
        );
      case 'MARKER_VIEW_UNKNOWN_AVAILABLE_ON_JP':
        return (
          'A indiqué une disponibilité INCONNUE sur Jean-Paul.io le ' +
          this.dateFormatted
        );
      case 'MARKER_VIEW_NOT_AVAILABLE_ON_JP':
        return (
          "S'est signalé NON DISPONIBLE sur Jean-Paul.io le " +
          this.dateFormatted
        );
