import User from './User'; // user class constructor
import Language from './Language';
import moment from 'moment/moment'; // date package
import FreelancePosition from './FreelancePosition'; // return roles details
import FreelanceEducation from './FreelanceEducation'; // reurn education details
import FreelanceCertification from './FreelanceCertification'; // return certification details
import DisponibilityMarker from './DisponibilityMarker'; // freelancer dispo details
import FreelanceSourcesPrices from './FreelanceSourcesPrices'; // tjm selon les sources
import CloudKeywords from './CloudKeywords'; // return keywords and hits
import FreelanceService from './FreelanceService'; // return headline service location
import Location from './Location'; // location details
...
class Freelance extends User {
  constructor({
    ...
    if (sources_prices) {
      this._sources_prices = sources_prices.map(
        (s) =>
          new FreelanceSourcesPrices({
            source: s.source,
            tjm: s.tjm,
            date: s.date,
          })
    ...
    if (disponibility_markers) {
      this._disponibility_markers = disponibility_markers.map(
        (dm) =>
          new DisponibilityMarker({
            code: dm.code,
    ...
    if (educations) {
      this._educations = educations.map(
        (education) => new FreelanceEducation(education)
      );
    }

    this._certifications = [];
    if (certifications) {
      this._certifications = certifications.map(
        (certification) => new FreelanceCertification(certification)
      );
    }
  ...
  get formattedPhones() {
    ...
    for (const phone of this._phones) {
      ...
      if (countryCode && countryCode.length > 0) {
        const phoneWithoutIndicator = clearedPhone
          .replace(/\(\+\d+\)/g, '')
          .replace(/[^\dA-Z]/g, '');
        if (phoneWithoutIndicator.length % 2 === 0) {
          clearedPhone =
            countryCode + ' ' + phoneWithoutIndicator.replace(/(.{2})/g, '$1 ');
        } else {
