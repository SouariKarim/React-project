...
import { UserContext } from '../contexts/UserContext'; // this context tells who is the user and if he is logged
import { useContext } from 'react';
import ReactGA from 'react-ga';

const useAuthManager = () => {
  const tokenStorage = useTokenStorage(); // has the methods returned from useTokenStorage
  const { setUser, isLogged, loggedUser } = useContext(UserContext); // data from the context

  const logFromAuthToken = ({ authToken }) => {
    tokenStorage.reset(); // clear all storage
  ...
  // logout and clear the storage and delete the user
  const logout = () => {
    tokenStorage.reset();
    setUser(null);
  };
  ...
  const isLoggedWithPremiumEsnAccount = () => {
    ...
    return (
      isEsn() &&
      (loggedUser.subscription_code === 'SMART' ||
        loggedUser.subscription_code === 'TOUT_PUISSANT')
    );
  };
