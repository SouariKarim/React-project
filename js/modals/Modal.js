...
export const ModalSubTitle = ({ children, className = '' }) => {
  return <p className={'modal-subtitle' + className}>{children}</p>;
};
    ...
    isShowing && (
      <ModalBootstrap
        ...
        className={
          className +
          ' modal-custom p-2' +
          (hideSeparation ? ' hide-separator' : '')
        }
      >
        <div className='modal-wrapper'>
          {/* if not loading render the passed children */}
          {children}
