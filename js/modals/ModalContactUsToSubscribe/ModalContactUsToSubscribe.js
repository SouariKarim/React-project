// a modal to contact us to subscribe
...
export default function ModalContactUsToSubscribe() {
  ...
  const askFreeTrial = optionContactUsToSubscribe?.askFreeTrial
    ? optionContactUsToSubscribe.askFreeTrial
    : false;
  const subscriptionCode = optionContactUsToSubscribe?.subscriptionCode
    ? optionContactUsToSubscribe.subscriptionCode
    : null;
    ...
    if (askFreeTrial === false) {
      ...
      if (user && askFreeTrial === false) {
        setText(
          '' +
            'Bonjour Jean-Michel,\n\n' +
            "Je souhaite upgrader mon compte en souscrivant à l'option payante " +
            subscriptionText +
            " et m'engage à régler dans les 48h après avoir reçu ma facture par email.\n\n" +
            'ID de mon compte : ' +
            user.id +
            '\n\n' +
  ...
  return (
    <ModalContactUs
      parentShowing={isShowingContactUsToSubscribe}
      parentToggle={toggleContactUsToSubscribe}
      text={text}
    />
  );
}
