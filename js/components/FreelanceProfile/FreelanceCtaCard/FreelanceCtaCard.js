// render the freelancer coord in the freelancer card
...
import { ModalContext } from '../../../contexts/ModalContext'; // methods for a modal
import useFreelancesApi from '../../../hooks/useFreelancesApi'; // request methods for freelancers
import useToast from '../../../hooks/useToast'; // create a toast
import useAuthManager from '../../../hooks/useAuthManager'; // auth methods

export default function FreelanceCtaCard({ freelance }) {
  const { toggleFreelanceContact } = useContext(ModalContext); // methods for toggling a freelancer contact modal
  const { isLogged } = useAuthManager();
  const { addToBookmark, removeFromBookmark, getResumeUrl } =
    useFreelancesApi();
  ...
  // render a toast and add a freelancer to the bookmark
  ...
  // render a toast after romoving a freelancer from the bookmark
          ...
          <Row className={'justify-content-end'}>
            ...
            >
              <Button
                ...
                onClick={() => toggleFreelanceContact({ freelance: freelance })}
              >
                Coordonnées
              </Button>
            </Col>
              ...
              <Dropdown className={'see-more-dropdown'}>
                ...
                <Dropdown.Menu>
                  ...
                  <Dropdown.Item
                    href={
                      freelance.hasResume
                        ? getResumeUrl({ freelance, absolute: true })
                        : null
                    }
                    target={'_blank'}
                    disabled={
                      !freelance.hasResume || !freelance.can_open_freelance
                    }
                  >
