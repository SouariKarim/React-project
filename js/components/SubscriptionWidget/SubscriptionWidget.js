// render a link pointing to the account informations like title expiration date etc
import './SubscriptionWidget.scss';
import Link from '../Link/Link'; // render a react-router-dom link
import useAuthManager from '../../hooks/useAuthManager'; // auth methods
import { useContext, useEffect } from 'react';
import { ModalContext } from '../../contexts/ModalContext'; // methods for displaying modal
...
export default function SubscriptionWidget() {
  ...
  const startAnimation =
    ...
    (user?.subscription.code === Constant.SUBSCRIPTION_CODE_RADIN ||
      user.subscription_expiration_days < 2);
  ...
  const lastTestEndEffectDate = user.lastTestDate
    ? user.lastTestDate.add(6, 'month')
    : null;
  ...
  if (isAdmin() === true) {
    ...
    if (user?.subscription.code === Constant.SUBSCRIPTION_CODE_RADIN) {
      ...
      if (lastTestEndEffectDate && moment().isBefore(lastTestEndEffectDate)) {
        ...
        lastTestEndEffectDate &&
        moment().isAfter(lastTestEndEffectDate)
      ) {
      ...
      if (isInTrialPeriod()) {
        account_details.expiration =
          expiration_timer !== 0
            ? `Expiration du test dans ${expiration_timer}`
            : 'Test expiré, passer à la version SMART ?';
      } else {
        account_details.expiration =
          expiration_timer !== 0
            ? `Expiration dans ${expiration_timer}`
            : 'Abonnement expiré, à renouveler';
      }
    }
  }
  ...
  if (isLogged() === true) {
    return (
      <Link
        ...
        to={
          isLoggedWithPremiumEsnAccount() && !isInTrialPeriod()
            ? 'CREDIT_MANAGE'
            : 'PRICING'
        }
      ...
      >
        <motion.p
          ...
          className={'info'}
        >
          {account_details.message} :<span>{account_details.title}</span>
        </motion.p>
