// import all the freelancer card component and render a full freelancer card

import React, { useContext, useState } from 'react';
import { Col, Row } from 'react-bootstrap';
import './FreelanceCard.scss';
import FreelanceProfilePicture from '../FreelanceProfilePicture/FreelanceProfilePicture'; // render the freelancer profile image
import {
...
import FreelanceSkill from '../FreelanceSkill/FreelanceSkill'; // return a highlighted skill keywords in a text conatining the skills
import FreelanceFunction from '../FreelanceFunction/FreelanceFunction'; // return highlighted keywords in a text containing the freelancer functions
import ViewStateIndicator from '../FreelanceIndicator/ViewStateIndicator/ViewStateIndicator'; // returns the freelancer state : new ....
import Button from '../Buttons/Button'; // a button component
import { ModalContext } from '../../contexts/ModalContext'; // returns methods for modals
import useAuthManager from '../../hooks/useAuthManager'; // returns auth methods
import { SearchFreelanceContext } from '../../contexts/SearchFreelanceContext'; // prvides the params and queries to search a freelancer
import HighlightText from '../Text/Text'; // return the highlighted text as a react element
import { FreelanceSectionCard } from '../FreelanceSectionCard/FreelanceSectionCard'; // returns the section of the card containing the title and the rest of this section
import FreelanceCtaCard from '../FreelanceProfile/FreelanceCtaCard/FreelanceCtaCard'; // render the freelancer coords in the freelancer card
import MaltIcon from '../Icon/MaltIcon';
import ApecIcon from '../Icon/ApecIcon';
import { DisplayFreelanceContext } from '../../contexts/DisplayFreelanceContext'; // methods for opening or closing the freelancer profile modal and lock or unlock the body
import DisponibilityMarker from '../DisponibilityMarker/DisponibilityMarker'; // render the disponibility marker
import InfoIconWrapper from '../InfoIconWrapper/InfoIconWrapper';
import ProfileTypeIndicator from '../FreelanceIndicator/ProfileTypeIndicator/ProfileTypeIndicator'; // render the freelance icon based on the freelance tag
import DoYourBuzzIcon from '../Icon/DoYourBuzzIcon';
import IconTooltip from '../IconTooltip/IconTooltip'; // render a tooltip
import CompanyIndicator from '../FreelanceIndicator/CompanyIndicator/CompanyIndicator'; // render the company indicator and the comments for the freelancer
import BookmarkIndicator from '../FreelanceIndicator/BookmarkIndicator/BookmarkIndicator'; // render if the freelancer is bookmarked
...
const FreelanceCard = ({
  ...
  const { toggleModalFreelancePremium, toggleFreelanceTJM } =
    useContext(ModalContext);
  const { canOpenFreelance, isLoggedWithPremiumEsnAccount, isCollaborator } =
    useAuthManager();
          ...
          <Row>
                  ...
                  {fromSearchPage === false ? (
                    <p className='freelance-title'>
                      <HighlightText
                        ...
                        highlightWords={
                          freelance.keywords_cloud
                            ? freelance.keywords_cloud.keywords
                            : []
                        }
                      />
                    </p>
            ...
            <Col xs={12} className={'freelance-localisation-container'}>
              <Row>
                <Col
                  xs={12}
                  xl={getLocationColumnSize().city}
                  className={'freelance-localisation-info text-truncate'}
                >
                  <FontAwesomeIcon
                    icon={freelance.isForeign ? faGlobe : LocationIcon}
                  />
                  {getLocationString()}
                </Col>
                ...
                {freelance.isRemote === true && (
                  <Col
                    xs={12}
                    xl={getLocationColumnSize().remote}
                    className={'freelance-localisation-info'}
                  >
                ...
                {freelance.mobilities.length > 0 && (
                  <Col
                    xs={12}
                    xl={getLocationColumnSize().mobilities}
                    className={'freelance-localisation-info text-truncate'}
                  >
                    {freelance.mobilities.length > 2 ? (
                      <IconTooltip
                        name={'mobilities'}
                        className={'tooltip-mobilities'}
                        htmlChildren={
                          <>
                            <FontAwesomeIcon icon={CardSideIcon} />
                            &nbsp;Mobilité :&nbsp;
                            {freelance.mobilities
                              .map(
                                (mobility) => mobility.mostAccurateLocationName
                              )
                              .join(', ')}
                          </>
                        }
                      >
                        {freelance.mobilities
                          .map((mobility) => mobility.mostAccurateLocationName)
                          .join(', ')}
                      </IconTooltip>
                    ) : (
                      <>
            ...
            <Col
              ...
              md={12}
              xl={11}
              className={'freelance-infos-container'}
            >
              <Row className={'freelance-infos'}>
                ...
                <Col
                  ...
                  md={4}
                  lg={2}
                  className={'freelance-info pb-2 pb-lg-0'}
                >
                ...
                >
                  ...
                  <p
                    className={!freelance.experience ? 'data no-data' : 'data'}
                  >
                ...
                >
                  ...
                  {isLoggedWithPremiumEsnAccount() === true ? (
                        ...
                        <>
                          ...
                          >
                            {isCollaborator() === true ? (
                              ...
                              <>
                                <p>
                                  Le TJM (Tarif Journalier Moyen) affiché
                                  provient des sources suivantes :
                                </p>
                                <ul>
                                  <li>
                                    TJM comuniqué directement par le freelance à
                                    Jean-michel.io
                                  </li>
                                  <li>TJM vu sur le web</li>
                                  <li>
                                    TJM indiqué par le freelance dans une
                                    réponse à annonce postée par Jean-Michel.io
                                  </li>
                                  <li>
                                    TJM communiqué par une ESN partenaire de
                                    Jean-Michel.io
                                  </li>
                                </ul>
                                <p style={{ margin: '5px 0 0 0' }}>
                                  Attention, les TJM annoncés par les freelances
                                  peuvent être très variables <br />
                    ...
                    <p>
                      <FontAwesomeIcon
                        onClick={() => toggleFreelanceTJM(true)}
                        className={'price-locked-icon'}
                        icon={faLock}
                      />
                    </p>
                  )}
