...
export default function Navbar({ withSideBar = false, theme = "light" }) {
    ...
    const {isLogged, logout, user, isLoggedWithPremiumEsnAccount, isCollaborator, isInTrialPeriod} = useAuthManager()
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (expandNavRef.current && !expandNavRef.current.contains(event.target) && event.target.className.includes && !event.target.className.includes("burger")) {
    ...
    return(
        <motion.header className={"custom-navbar " + navTheme}
            ...
            <nav ref={expandNavRef} className={(open)? "open" : undefined}>
                {isLogged() ?
                    <>
                        <Button variant={"navlink " + navTheme} activeClassName={"active"} to={"SEARCH"}>CVthèque</Button>

                        {(withSideBar === false || isCollaborator() === true || isInTrialPeriod() === true || isLoggedWithPremiumEsnAccount() === false) &&
                            <Button variant={"navlink " + navTheme} activeClassName={"active"} to={"PRICING"}>Tarifs</Button>
                        }

                        <NavDropdown title={user.full_name} id="nav-dropdown">
                            <NavDropdown.Item onClick={onMyAccountClick}>Mon compte</NavDropdown.Item>
                            <NavDropdown.Divider/>
                            <NavDropdown.Item onClick={onLogoutClick}>Déconnexion</NavDropdown.Item>
                        </NavDropdown>
                    </>
                    :
                    <>
                        <Button variant={"navlink " + navTheme} activeClassName={"active"} to={"PRICING"}>Tarifs</Button>
                        <Button variant={"navlink " + navTheme} onClick={toggleLogin}>Connexion</Button>
                        <Button variant={"cta secondary"} style={{marginLeft: 20}} onClick={toggleRegistration}>
            ...
            <Burger active={open} theme={navTheme} className={"burger"}
