...
import ModalConfirmDeleteAccount from "../../modals/ModalConfirmDeleteAccount/ModalConfirmDeleteAccount";
...
const UserSchema = Yup.object().shape({
    ...
    confirmPassword:
        Yup.string().when('plainPassword', {
            is: (password) => password && password.length > 0,
            then: Yup.string().test('passwords-match', 'Les mots de passe doivent correspondre', function (value) {
...
const MyAccount = () => {
    ...
    const [showingConfirm, toggleConfirm] = useModal();
    ...
    const onSubmit = (values, {setSubmitting, setFieldError}) => {
        ...
        toast.promise({
            promise: godzilla.usersApi.updateUser({id: user.id, ...valuesFormatted}),
            ...
            .catch(errors => {
                if (Array.isArray(errors)) {
                    ...
                    setFieldError('global', 'Une erreur est survenue lors de la modification.')
                }
            })
    ...
    return (
        <Container className={"my-account"} fluid>
            <ModalConfirmDeleteAccount toggle={toggleConfirm} isShowing={showingConfirm}/>
            <Row>
                <Col xs={12} md={9}>
                    <Card>
                        <Card.Body>
                            ...
                            >
                                {({errors, touched, isSubmitting, isValid, dirty}) => (
                                    <Form>
                                        ...
                                        <FormGroup controlId="plainPassword">
                                            <InputGroup>
                                                <Input name={"plainPassword"}
                                                       ...
                                                       autoComplete={"false"}
                                                       isValid={dirty && touched.plainPassword && !errors.plainPassword}
                                                       isInvalid={dirty && touched.plainPassword && errors.plainPassword}
                                                />
                                                <InputError><p>{errors.plainPassword}</p></InputError>
                                            </InputGroup>
                                        </FormGroup>
                                        ...
                                        {errors.global ?
                                            <div className={"form-error"}><p>{errors.global}</p></div> : null}
