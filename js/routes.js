...
export const ROUTES = [
    ...
    },
    {
        ...
        guard: ({userAuthManager}) => userAuthManager.isLogged(),
        redirect: '/'
    },
    {
        path: '/company/credit',
    ...
    },
    {
        ...
        guard: ({userAuthManager}) => userAuthManager.isLogged() === false,
        redirect: '/'
    },
    {
        path: '/search',
        ...
        if (route.guard && route.guard({userAuthManager}) === false) {
            return (
                <Route
                    key={route.key + index}
                    element={
                        <Navigate to={route.redirect ? route.redirect : '/'} replace={true}/>
                    }
                />
            )
        }
        else {
            const Layout = route.layout ? route.layout : SiteLayout
            return (
                <Route
